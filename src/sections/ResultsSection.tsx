    ...
    <section id="results" className="py-20 px-4 sm:px-6 lg:px-8 bg-surface">
      <div className="max-w-[1440px] mx-auto">
        ...
        >
          ...
          <p className="text-xl text-[color:var(--text-secondary)]">{t('results.subtitle')}</p>
        </motion.div>
          ...
          {metrics.map((metric, index) => (
            <motion.div key={index} variants={itemVariants}>
              <Card hover className="text-center h-full flex flex-col justify-center">
                ...
                <p className="text-[color:var(--text-secondary)] text-lg font-medium">{metric.label}</p>
              </Card>
            </motion.div>
          ))}
        </motion.div>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <p className="text-lg text-[color:var(--text-secondary)] italic mb-4">
            &ldquo;{t('results.quoteText')}&rdquo;
          </p>
          <p className="text-sm font-semibold text-primary">{t('results.quoteAuthor')}</p>
        </motion.div>
      </div>
    </section>
  )
